...
def robust_zscore(data, quantiles=[0.25, 0.75], unit_variance=True, per_channel_variance=True, axis=1):
    '''
        Robust z-scoring like sklearn's RobustScaler
        Remove median per channel and rescale with overall variance across space
        assume data is (space x time)
    '''
    data = data - np.nanmedian(data, axis=axis, keepdims=True)

    limits = np.nanquantile(data, quantiles, axis=axis, keepdims=True)
    if not per_channel_variance:
        limits = limits.mean(axis=1, keepdims=True)
    scale = np.diff(limits, axis=0)[0]
    if unit_variance:
        adjust = np.diff(scipy.stats.norm.ppf(quantiles), axis=0)[0]
    scale = scale / adjust
    return data / scale
